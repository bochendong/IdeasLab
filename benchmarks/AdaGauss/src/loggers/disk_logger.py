...
class Logger(ExperimentLogger):
    ...
    def log_result(self, array, name, step):
        ...
        np.savetxt(os.path.join(results_dir, '{}-{}.txt'.format(name, self.begin_time_str)),
                   array, '%.6f', delimiter='\t')

    def log_figure(self, name, iter, figure, curtime=None):
        curtime = datetime.now()
        figures_dir = os.path.join(self.exp_path, 'figures')
        os.makedirs(figures_dir, exist_ok=True)
        figure.savefig(os.path.join(figures_dir,
    ...
    def save_model(self, state_dict, task):
        models_dir = os.path.join(self.exp_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        torch.save(state_dict, os.path.join(models_dir, "task{}.ckpt".format(task)))
