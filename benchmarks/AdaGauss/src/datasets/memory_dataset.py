...
def get_data(trn_data, tst_data, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    n_unique_trn_before = len(np.unique(np.asarray(trn_data['y'])))
    ...
    for tt in range(num_tasks):
        ...
        if data[tt]['ncla'] != cpertask[tt]:
            msg = (
                "something went wrong splitting classes: "
                "num_tasks=%s, nc_first_task=%s, num_classes(from class_order)=%s, "
                "unique_labels_in_train=%s, expected_per_task=%s; "
                "task %s has ncla=%s (expected %s)."
            ) % (num_tasks, nc_first_task, num_classes, n_unique_trn_before, cpertask.tolist(), tt, data[tt]['ncla'], cpertask[tt])
