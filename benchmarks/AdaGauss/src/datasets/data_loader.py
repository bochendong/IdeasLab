...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    # 必须先判 cifar100：'cifar10' in 'cifar100_icarl' 为 True 会误入 cifar10 分支
        ...
        # 保证 y 为 list of int，避免 tensor/np.scalar 导致 task 划分断言失败
        trn_data = {'x': np.asarray(tvcifar_trn.data), 'y': [int(y) for y in tvcifar_trn.targets]}
        tst_data = {'x': np.asarray(tvcifar_tst.data), 'y': [int(y) for y in tvcifar_tst.targets]}
        n_cls = len(np.unique(trn_data['y']))
        if n_cls != 100:
            raise RuntimeError(
                "CIFAR-100 应含 100 类，当前只读到 %s 类（路径: %s）。"
                "请删掉该路径对应文件夹后重跑，将自动下载 CIFAR-100。" % (n_cls, path)
            )
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
                                                         num_tasks=num_tasks, nc_first_task=nc_first_task,
