# ===== Exp18: EFC 风格非对称 CE =====
"""对旧类 logit 降权，减轻新类主导，缓解 slice 失衡。"""
import os
import sys
import copy
import random
...
def asymmetric_ce_loss(logits, y, num_old_classes, new_weight=0.8, device=DEVICE):
    """新类 logit 乘 new_weight（<1），减轻新类主导、平衡 slice。"""
    if num_old_classes <= 0 or num_old_classes >= logits.size(1):
        return F.cross_entropy(logits, y)
    logits_scaled = logits.clone()
    logits_scaled[:, num_old_classes:] = logits_scaled[:, num_old_classes:] * new_weight
    return F.cross_entropy(logits_scaled, y)


def run_experiment_asymmetric_ce(
    run_name: str = "exp18_asymmetric_ce",
    config: dict = None,
    save_model_checkpoint: bool = False,
    script_file: str = None,
    new_weight: float = 0.8,
):
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        for task in range(5):
            ...
            num_old_classes = 2 * task if task > 0 else 0
            step = 0
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    logits, feat = model(x)
                    c_loss = asymmetric_ce_loss(logits[:, :valid_out_dim], y, num_old_classes, new_weight, device=DEVICE)
...
if __name__ == "__main__":
    run_experiment_asymmetric_ce(
        run_name="exp18_asymmetric_ce",
