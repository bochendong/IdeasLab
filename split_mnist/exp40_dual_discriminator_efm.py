# ===== Exp40: 双判别器 + EFM 特征漂移正则（EFC 风）=====
"""Exp32 + 对伪旧样本 (f_new - f_old)^T (lambda E + eta I)(f_new - f_old)，E 为上一任务特征方差（对角近似）。"""
import os
import sys
import copy
import logging
...
def compute_efm_diagonal(model, task_loaders, task_id, device, max_samples=500):
    """当前 model 在 task_id 数据上的特征，按维度算方差，返回 (400,) 向量 E。"""
    model.eval()
    feats = []
    with torch.no_grad():
        for x, y in task_loaders[task_id][0]:
            ...
            if sum(f.size(0) for f in feats) >= max_samples:
                break
    if not feats:
        return torch.ones(400, device=device)
    F_all = torch.cat(feats, dim=0)[:max_samples]
    return F_all.var(dim=0) + 1e-8


def run_experiment(run_name="exp40_dual_discriminator_efm", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_efm=0.1, eta_efm=0.01):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "lambda_efm": lambda_efm, "eta_efm": eta_efm}
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        for task in range(5):
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            if E_list:
                                with torch.no_grad():
                                    _, f_old_ref = prev_models[-1](x_fake)
                                E_prev = E_list[-1].to(DEVICE)
                                drift = feat_old_for_adv - f_old_ref
                                loss_efm = (drift.pow(2) * (lambda_efm * E_prev + eta_efm)).sum(dim=1).mean()
                                loss = loss + loss_efm
            ...
            E_list.append(compute_efm_diagonal(model, task_loaders, task, DEVICE))
...
if __name__ == "__main__":
    run_experiment(run_name="exp40_dual_discriminator_efm", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_efm=0.1, eta_efm=0.01)
