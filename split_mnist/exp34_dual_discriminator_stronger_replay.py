# ===== Exp34: 双判别器 + 加强伪回放 =====
"""Exp32 配置，vae_n_fake_per_task=64、每 batch 伪样本上限 256、vae_epochs=8。"""
import os
import sys
import copy
import logging
...
FAKE_CAP = 256  # 每 batch 伪样本上限（Exp32 为 128）


def run_experiment(run_name="exp34_dual_discriminator_stronger_replay", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=8, vae_n_fake_per_task=64, lambda_adv_feat=0.1, lambda_adv_slice=0.1):
...
if __name__ == "__main__":
    run_experiment(run_name="exp34_dual_discriminator_stronger_replay", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=8, vae_n_fake_per_task=64,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1)
