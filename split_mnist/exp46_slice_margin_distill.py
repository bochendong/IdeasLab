# ===== Exp46: Exp35 + 伪旧样本软标签蒸馏 =====
"""双判别器 + slice margin + 伪旧样本上 KL(当前 logits ∥ 上一任务模型 logits)，保持旧类 logit 分布一致。"""
import os
import sys
import copy
import logging
...
def run_experiment(run_name="exp46_slice_margin_distill", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_distill=0.5, distill_T=2.0):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "lambda_distill": lambda_distill, "distill_T": distill_T}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            # 软标签蒸馏：上一任务模型作 teacher
                            with torch.no_grad():
                                teacher_logits = prev_models[task - 1](x_fake)[0][:, : 2 * task]
                            student_logits = logits_fake2[:, : 2 * task]
                            loss = loss + lambda_distill * F.kl_div(
                                F.log_softmax(student_logits / distill_T, dim=1),
                                F.softmax(teacher_logits / distill_T, dim=1),
                                reduction="batchmean",
                            ) * (distill_T * distill_T)
...
if __name__ == "__main__":
    run_experiment(run_name="exp46_slice_margin_distill", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_distill=0.5, distill_T=2.0)
