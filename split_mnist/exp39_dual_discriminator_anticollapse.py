# ===== Exp39: 双判别器 + Anti-collapse（AdaGauss 风）=====
"""Exp32 + 对伪旧样本特征（或 slice）批协方差做 anti-collapse，缓解维度坍缩与 task-recency bias。"""
import os
import sys
import copy
import logging
...
def run_experiment(run_name="exp39_dual_discriminator_anticollapse", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_ac=0.1, ac_min_std=1.0):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "lambda_ac": lambda_ac, "ac_min_std": ac_min_std}
...
if __name__ == "__main__":
    run_experiment(run_name="exp39_dual_discriminator_anticollapse", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_ac=0.1, ac_min_std=1.0)
