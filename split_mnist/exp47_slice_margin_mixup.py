# ===== Exp47: Exp35 + Mixup（真实新 + 伪旧）=====
"""双判别器 + slice margin + 真实新任务样本与伪旧样本做 Mixup，软标签 CE，平滑新旧边界。"""
import os
import sys
import copy
import logging
...
def run_experiment(run_name="exp47_slice_margin_mixup", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, mix_alpha=0.4, lambda_mix=0.5):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "mix_alpha": mix_alpha, "lambda_mix": lambda_mix}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            # Mixup 真实新 + 伪旧，软标签 CE
                            N = min(x.size(0), x_fake.size(0))
                            if N > 0:
                                lam = np.random.beta(mix_alpha, mix_alpha)
                                x_mix = lam * x[:N] + (1 - lam) * x_fake[:N]
                                one_hot_real = F.one_hot(y[:N], valid_out_dim).float()
                                one_hot_fake = F.one_hot(y_fake[:N], valid_out_dim).float()
                                soft_y = lam * one_hot_real + (1 - lam) * one_hot_fake
                                logits_mix, _ = model(x_mix)
                                logits_mix = logits_mix[:, :valid_out_dim]
                                loss_mix = -(soft_y * F.log_softmax(logits_mix, dim=1)).sum(dim=1).mean()
                                loss = loss + lambda_mix * loss_mix
...
if __name__ == "__main__":
    run_experiment(run_name="exp47_slice_margin_mixup", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, mix_alpha=0.4, lambda_mix=0.5)
