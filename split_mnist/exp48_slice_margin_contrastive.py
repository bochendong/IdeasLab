# ===== Exp48: Exp35 + 对比学习（原型作 anchor）=====
"""双判别器 + slice margin + 伪旧样本特征与存储的类原型做 SupCon：拉近同类别、推远异类，稳定旧类表征。"""
import os
import sys
import copy
import logging
...
from split_mnist.exp36_dual_discriminator_proto import compute_prototypes


def slice_margin_loss(logits_fake, y_fake, num_tasks_old, device, margin=0.3):
    if logits_fake.size(0) == 0:
        return torch.tensor(0.0, device=device)
    slice_max_vec = slice_max_vector(logits_fake, num_tasks_old, device)
    idx = (y_fake // 2).clamp(0, slice_max_vec.size(1) - 1)
    correct_max = slice_max_vec.gather(1, idx.unsqueeze(1)).squeeze(1)
    sm = slice_max_vec.clone()
    sm.scatter_(1, idx.unsqueeze(1), -1e9)
    best_other = sm.max(dim=1).values
    gap = correct_max - best_other
    return F.relu(margin - gap).mean()


def run_experiment(run_name="exp48_slice_margin_contrastive", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_contrast=0.5, contrast_tau=0.1):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "lambda_contrast": lambda_contrast, "contrast_tau": contrast_tau}
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        prev_models, vaes = [], []
        all_prototypes = []  # list of [400] per class
        ...
        for task in range(5):
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            loss = loss + lambda_margin * slice_margin_loss(logits_fake2, y_fake, num_tasks_old, DEVICE, margin=slice_margin)

                            # 对比学习：伪旧特征与类原型，SupCon 风（CE 到原型 logits）
                            if len(all_prototypes) >= 2 * task:
                                feat_norm = F.normalize(feat_old_for_adv, dim=1)
                                proto_stack = torch.stack([all_prototypes[i].to(DEVICE) for i in range(2 * task)])
                                proto_norm = F.normalize(proto_stack, dim=1)
                                logits_sup = (feat_norm @ proto_norm.t()) / contrast_tau
                                loss = loss + lambda_contrast * F.cross_entropy(logits_sup, y_fake)
            ...
            protos = compute_prototypes(model, task_loaders, task, DEVICE)
            for c in sorted(protos.keys()):
                all_prototypes.append(protos[c].detach().clone())
...
if __name__ == "__main__":
    run_experiment(run_name="exp48_slice_margin_contrastive", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_contrast=0.5, contrast_tau=0.1)
