# ===== Exp42: 双判别器 + Slice 模式一致性（TASS 风）=====
"""Exp32 + 伪旧样本上当前 slice 激活模式与学该任务时保存的模板一致。"""
import os
import sys
import copy
import logging
...
def compute_mean_slice_pattern(model, task_loaders, task_id, device, num_tasks):
    model.eval()
    patterns = []
    with torch.no_grad():
        for x, y in task_loaders[task_id][0]:
            x = x.to(device)
            logits, _ = model(x)
            logits = logits[:, :2 * (task_id + 1)]
            sm = slice_max_vector(logits, task_id + 1, device)
            patterns.append(sm)
    if not patterns:
        return torch.zeros(num_tasks, device=device)
    out = torch.cat(patterns, dim=0).mean(dim=0)
    pad = F.pad(out.unsqueeze(0), (0, num_tasks - out.size(0)), value=0.0).squeeze(0)
    return pad


def run_experiment(run_name="exp42_dual_discriminator_slice_consist", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_consist=0.2):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "lambda_consist": lambda_consist}
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        for task in range(5):
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            if stored_slice_pattern:
                                stacked = torch.stack(stored_slice_pattern).to(DEVICE)
                                task_id = (y_fake // 2).clamp(0, stacked.size(0) - 1)
                                target = stacked[task_id]
                                current_pad = F.pad(slice_max_old_adv, (0, NUM_TASKS - slice_max_old_adv.size(1)), value=0.0)
                                loss = loss + lambda_consist * F.mse_loss(current_pad, target)
            ...
            stored_slice_pattern.append(compute_mean_slice_pattern(model, task_loaders, task, DEVICE, NUM_TASKS))
...
if __name__ == "__main__":
    run_experiment(run_name="exp42_dual_discriminator_slice_consist", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_consist=0.2)
