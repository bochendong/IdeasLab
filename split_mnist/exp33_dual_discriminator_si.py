# ===== Exp33: 双判别器 + SI =====
"""Exp32（特征+slice 双判别器 + VAE 伪回放）+ Synaptic Intelligence。"""
import os
import sys
import copy
import logging
...
from split_mnist.exp32_dual_discriminator import (
    FeatDiscriminator, SliceDiscriminator, slice_max_vector, NUM_TASKS,
)

def run_experiment(run_name="exp33_dual_discriminator_si", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, si_lambda=1.0):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "si_lambda": si_lambda}
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        for task in range(5):
            ...
            if si_lambda > 0:
                si_omega_star_list.append(({k: v.clone() for k, v in si_omega.items()}, {k: v.clone() for k, v in si_theta_prev.items()}))
...
if __name__ == "__main__":
    run_experiment(run_name="exp33_dual_discriminator_si", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, si_lambda=1.0)
