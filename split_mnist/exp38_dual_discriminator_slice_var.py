# ===== Exp38: 双判别器 + 旧类 slice 强度方差正则 =====
"""在伪旧样本上约束各 slice 强度方差不要过大，避免新 slice 碾压旧 slice。"""
import os
import sys
import copy
import logging
...
def run_experiment(run_name="exp38_dual_discriminator_slice_var", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_slice_var=0.1):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "lambda_slice_var": lambda_slice_var}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            loss_var = slice_max_old_adv.var(dim=1).mean()
                            loss = loss + lambda_slice_var * loss_var
...
if __name__ == "__main__":
    run_experiment(run_name="exp38_dual_discriminator_slice_var", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, lambda_slice_var=0.1)
