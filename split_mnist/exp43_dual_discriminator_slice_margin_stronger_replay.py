# ===== Exp43: Exp35 + Exp34 = 双判别器 + Slice margin + 加强伪回放 =====
"""Exp35（slice margin） + Exp34 配置：vae_n_fake_per_task=64、fake_cap=256、vae_epochs=8。目标冲 52%+。"""
import os
import sys
import copy
import logging
...
FAKE_CAP = 256  # 每 batch 伪样本上限（Exp35 为 128）
...
def run_experiment(run_name="exp43_dual_discriminator_slice_margin_stronger_replay", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=8, vae_n_fake_per_task=64, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "vae_epochs": vae_epochs, "vae_n_fake_per_task": vae_n_fake_per_task, "fake_cap": FAKE_CAP,
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice,
        "slice_margin": slice_margin, "lambda_margin": lambda_margin}
        ...
        for task in range(5):
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        n_fake = min(vae_n_fake_per_task * task, FAKE_CAP)
...
if __name__ == "__main__":
    run_experiment(run_name="exp43_dual_discriminator_slice_margin_stronger_replay", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=8, vae_n_fake_per_task=64,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5)
