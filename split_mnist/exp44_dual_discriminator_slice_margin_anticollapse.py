# ===== Exp44: Exp35 + Exp39 = 双判别器 + Slice margin + Anti-collapse =====
"""Exp35（slice margin） + Exp39（特征/slice 批协方差 anti-collapse）。目标冲 52%+。"""
import os
import sys
import copy
import logging
...
from split_mnist.exp22_slice_gauss_anticollapse import anti_collapse_loss
...
def slice_margin_loss(logits_fake, y_fake, num_tasks_old, device, margin=0.3):
    """L = mean(max(0, margin - (correct_slice_max - best_other_max)))."""
    if logits_fake.size(0) == 0:
        return torch.tensor(0.0, device=device)
    slice_max_vec = slice_max_vector(logits_fake, num_tasks_old, device)  # [B, T]
...
def run_experiment(run_name="exp44_dual_discriminator_slice_margin_anticollapse", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_ac=0.1, ac_min_std=1.0):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "lambda_ac": lambda_ac, "ac_min_std": ac_min_std}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            loss = loss + lambda_ac * (anti_collapse_loss(feat_old_for_adv, DEVICE, min_std=ac_min_std) + anti_collapse_loss(slice_max_old_adv, DEVICE, min_std=ac_min_std))
...
if __name__ == "__main__":
    run_experiment(run_name="exp44_dual_discriminator_slice_margin_anticollapse", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, lambda_ac=0.1, ac_min_std=1.0)
