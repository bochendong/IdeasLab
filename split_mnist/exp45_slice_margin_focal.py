# ===== Exp45: Exp35 + Focal Loss =====
"""双判别器 + slice margin，CE 改为 Focal Loss（难样本权重大），易忘样本自动获得更多梯度。"""
import os
import sys
import copy
import logging
...
def focal_loss(logits, targets, gamma=2.0, reduction="mean"):
    """Focal: -alpha_t * (1-p_t)^gamma * log(p_t). 此处 alpha=1。"""
    log_p = F.log_softmax(logits, dim=1)
    pt = log_p.gather(1, targets.unsqueeze(1)).squeeze(1).exp()
...
def run_experiment(run_name="exp45_slice_margin_focal", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, focal_gamma=2.0):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "focal_gamma": focal_gamma}
        ...
        for task in range(5):
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    logits, feat = model(x)
                    c_loss = focal_loss(logits[:, :valid_out_dim], y, gamma=focal_gamma)
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            loss = loss + 0.5 * focal_loss(logits_fake2, y_fake, gamma=focal_gamma)
...
if __name__ == "__main__":
    run_experiment(run_name="exp45_slice_margin_focal", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, focal_gamma=2.0)
