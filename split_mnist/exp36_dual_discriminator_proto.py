# ===== Exp36: 双判别器 + 原型伪回放 =====
"""Exp32 + Exp19 风格：每类存特征原型，新任务时对原型加噪在 head 上做 CE（与 VAE 伪样本 CE 并存）。"""
import os
import sys
import copy
import logging
...
def logits_from_feat(model, feat):
    feat_n = F.normalize(feat, dim=1)
    W_n = F.normalize(model.W, dim=1)
    return model.logit_scale * (feat_n @ W_n.t())


def run_experiment(run_name="exp36_dual_discriminator_proto", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, proto_aug_r=0.4, proto_aug_n=8):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "proto_aug_r": proto_aug_r, "proto_aug_n": proto_aug_n}
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if protos_by_task:
                            pf, pl = [], []
                            for protos in protos_by_task:
                                pfi, pli = sample_augmented_prototypes(protos, proto_aug_n, proto_aug_r, DEVICE)
                                if pfi is not None:
                                    pf.append(pfi)
                                    pl.append(pli)
                            if pf:
                                proto_feat = torch.cat(pf, dim=0)
                                proto_label = torch.cat(pl, dim=0)
                                logits_proto = logits_from_feat(model, proto_feat)
                                loss = loss + 0.5 * criterion(logits_proto[:, :valid_out_dim], proto_label)
...
if __name__ == "__main__":
    run_experiment(run_name="exp36_dual_discriminator_proto", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, proto_aug_r=0.4, proto_aug_n=8)
