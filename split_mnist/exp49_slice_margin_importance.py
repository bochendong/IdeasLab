# ===== Exp49: Exp35 + 重要性加权 =====
"""双判别器 + slice margin + 伪回放损失按任务加权：越早的任务（越易忘）权重越大，decay^task_id。"""
import os
import sys
import copy
import logging
...
def slice_margin_loss_per_sample(logits_fake, y_fake, num_tasks_old, device, margin=0.3):
    """Per-sample margin loss, shape [B]."""
    ...
    return F.relu(margin - gap)  # [B]


def run_experiment(run_name="exp49_slice_margin_importance", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, importance_decay=0.9):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "slice_margin": slice_margin, "lambda_margin": lambda_margin, "importance_decay": importance_decay}
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            task_ids = y_fake // 2  # 0,0,1,1,... 越早任务 id 越小
                            w = (importance_decay ** task_ids.float()).to(DEVICE)  # 越早权重越大
                            ...
                            ce_fake = F.cross_entropy(logits_fake2, y_fake, reduction="none")  # [N]
                            loss = loss + 0.5 * (w * ce_fake).mean()
                            margin_per = slice_margin_loss_per_sample(logits_fake2, y_fake, num_tasks_old, DEVICE, margin=slice_margin)
                            loss = loss + lambda_margin * (w * margin_per).mean()
...
if __name__ == "__main__":
    run_experiment(run_name="exp49_slice_margin_importance", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5, importance_decay=0.9)
