# ===== Exp37: 双判别器 + 50% 伪回放每 batch =====
"""每个 batch 强制 50% 真实新任务 + 50% 伪旧，基于 Exp32。"""
import os
import sys
import copy
import logging
...
def run_experiment(run_name="exp37_dual_discriminator_balanced_batch", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=5, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, half_batch_size=64):
    ...
    n_real = half_batch_size
    n_fake_target = half_batch_size
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "half_batch_size": half_batch_size}
        ...
        for task in range(5):
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    x_real, y_real = x[:n_real], y[:n_real]
                    if task > 0 and vaes:
                        n_fake = min(n_fake_target, vae_n_fake_per_task * task)
                        x_fake, y_fake = sample_from_vaes(vaes[:-1], list(range(task)), max(1, n_fake // task), DEVICE)
                        x_fake = x_fake.view(-1, 1, 28, 28)
                        if x_fake.size(0) > n_fake_target:
                            x_fake, y_fake = x_fake[:n_fake_target], y_fake[:n_fake_target]
                        elif x_fake.size(0) < n_fake_target and x_fake.size(0) > 0:
                            pass
                    ...
                    logits, feat = model(x_all)
                    c_loss = criterion(logits[:, :valid_out_dim], y_all)
                    r_loss = reg_loss_slice(logits, feat, x_all, prev_models, task, lambda_slice, lambda_feat, device=DEVICE)
                    loss = c_loss + r_loss

                    if task > 0 and vaes and x_fake.size(0) > 0:
                        feat_new = feat[:n_real].detach()
                        feat_old = feat[n_real:].detach()
                        slice_max_new = slice_max_vector(logits[:n_real], num_tasks_cur, DEVICE)
                        slice_max_old = slice_max_vector(logits[n_real:], task, DEVICE)
                        ...
                        slice_max_old_adv = slice_max_vector(logits_fake2, task, DEVICE)
...
if __name__ == "__main__":
    run_experiment(run_name="exp37_dual_discriminator_balanced_batch", config={"lambda_slice": 2.0, "lambda_feat": 0.5},
        script_file=os.path.abspath(__file__), vae_epochs=5, vae_n_fake_per_task=32,
        lambda_adv_feat=0.1, lambda_adv_slice=0.1, half_batch_size=64)
