# ===== MNIST Baseline 超参扫描：先调出较好 baseline，再固定 lr/epochs 跑其他方法 =====
# 用法（项目根目录）：python split_mnist/tune_baseline.py
# 可选：python split_mnist/tune_baseline.py --lr 0.003 0.005 --ep 4 6
import os
import sys
import json
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Tune MNIST baseline (lr, epochs_per_task). Same config then used for all other methods.")
    parser.add_argument("--lr", nargs="*", type=float, default=[3e-3, 5e-3, 1e-2], help="Learning rates to try.")
    parser.add_argument("--ep", nargs="*", type=int, default=[4, 6, 8], help="Epochs per task to try.")
    ...
    for c in configs:
        ...
        run_name = f"mnist_baseline_tune_{tune_id}_lr{lr}_ep{ep}"
        print(f"\n>> Running {run_name}")
        run_experiment(run_name=run_name, config=c, save_model_checkpoint=False, script_file=os.path.join(script_dir, "tune_baseline.py"))

    exp_root = os.path.join(_PROJECT_ROOT, "output", "split_mnist", "experiments")
    rows = []
    ...
    rows.sort(key=lambda x: (-x["class_il"], x["bwt"]))

    print("\n" + "=" * 60)
    print("BASELINE TUNE SUMMARY (higher Class-IL better)")
    print("=" * 60)
    if not rows:
        print("No metrics found. Check output/split_mnist/experiments/")
        return
    for r in rows:
