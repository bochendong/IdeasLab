# ===== Exp17: LDC 风格漂移补偿（Drift Compensator）=====
"""添加 projector 将当前特征映射到上一任务特征空间，MSE(P(feat), teacher_feat) 作为辅助损失，缓解 backbone 漂移。"""
import os
import sys
import copy
import random
...
class Projector(nn.Module):
    """单层 MLP: 400 -> 400，将当前特征映射到上一任务特征空间"""
...
def run_experiment_ldc(
    run_name: str = "exp17_ldc_drift",
    config: dict = None,
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_mnist") as mgr:
        ...
        projector = Projector(dim=400).to(DEVICE)
        prev_models = []  # 上一任务的 teacher
        prev_projector = None  # 上一任务的 projector（推理时不用）
        ...
        for task in range(5):
            ...
            # 每任务重新初始化 projector（学习从当前 backbone 到上一任务空间的映射）
            if task > 0:
                projector = Projector(dim=400).to(DEVICE)
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    # LDC: 学习 projector 使 P(current_feat) ≈ teacher_feat
                    ldc_loss = torch.tensor(0.0, device=DEVICE)
                    if task > 0 and prev_models:
                        with torch.no_grad():
                            _, teacher_feat = prev_models[-1](x)
                        projected = projector(feat)
                        ldc_loss = F.mse_loss(projected, teacher_feat)
                        loss = loss + lambda_ldc * ldc_loss
                    ...
                    if step % 25 == 0:
                        ...
                        if ldc_loss.item() > 0:
                            msg.append(f"ldc={ldc_loss.item():.4f}")
            ...
            prev_projector = copy.deepcopy(projector).to(DEVICE) if task > 0 else None
