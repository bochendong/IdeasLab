# Split CIFAR-10 双判别器 + Slice margin（对应 MNIST Exp35）
import os
import sys
import copy
import logging
...
def slice_margin_loss_fake(logits_fake, y_fake, num_tasks_old, device, margin=0.3):
...
def run_experiment(run_name="cifar10_slice_margin", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=8, vae_n_fake_per_task=32, lambda_adv_feat=0.1, lambda_adv_slice=0.1, slice_margin=0.3, lambda_margin=0.5):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"], "batch_size": cfg["batch_size"],
        ...
        "lambda_adv_feat": lambda_adv_feat, "lambda_adv_slice": lambda_adv_slice, "slice_margin": slice_margin, "lambda_margin": lambda_margin}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            mrg_loss = slice_margin_loss_fake(logits_fake2, y_fake, num_tasks_old, DEVICE, margin=slice_margin)
                            loss = loss + lambda_adv_feat * loss_adv_feat + lambda_adv_slice * loss_adv_slice + lambda_margin * mrg_loss + 0.5 * criterion(logits_fake2, y_fake)
...
if __name__ == "__main__":
    run_experiment(run_name="cifar10_slice_margin", script_file=os.path.abspath(__file__))
