# Split CIFAR-10 当前任务 slice margin + SI（无 replay）
# 思路：在真实当前任务数据上显式约束「正确 slice 比其它 slice 大 margin」，使 winner=task；配合 SI 保持旧参数，无伪样本。
import os
import sys
import copy
import logging
...
from split_cifar10.common import (
    TASK_DIR, build_all_task_loaders, cal_acc_class_il,
...
def margin_loss_current_batch(logits, y, task_id, num_tasks_cur, margin=0.3, device=DEVICE):
    """当前 batch 上：正确 slice 的 max 应比其它 slice 的 max 至少大 margin。"""
...
def run_experiment(run_name="cifar10_current_margin_si", config=None, save_model_checkpoint=False, script_file=None,
    si_lambda=0.5, lambda_margin=0.5, slice_margin=0.3):
    ...
    exp_config = {
        ...
        "si_lambda": si_lambda, "lambda_margin": lambda_margin, "slice_margin": slice_margin, "seed": cfg["seed"],
    }
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_cifar10") as mgr:
        ...
        logging.info("Current-task slice margin + SI, no replay. Explicit gap on real current-task data.")
        ...
        for task in range(5):
            ...
            for ep in range(cfg["epochs_per_task"]):
                for x, y in task_loaders[task][0]:
                    ...
                    loss = c_loss + r_loss + m_loss + si_loss
                    optimizer.zero_grad()
                    loss.backward()
                    ...
                    if step % 50 == 0:
                        msg = [f"[Task {task} | ep {ep} | step {step}] loss={loss.item():.4f} c={c_loss.item():.4f} m={m_loss.item():.4f}"]
                        for t in range(task + 1):
                            acc_ci = cal_acc_class_il(model, task_loaders[t][0], valid_out_dim, device=DEVICE)
                            msg.append(f"CI_T{t}={acc_ci*100:.1f}%")
...
if __name__ == "__main__":
    run_experiment(run_name="cifar10_current_margin_si", script_file=os.path.abspath(__file__))
