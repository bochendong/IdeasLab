# Split CIFAR-10 VAE 伪回放 + SI（对应 MNIST Exp27）
import os
import sys
import copy
import logging
...
def run_experiment(run_name="cifar10_vae_si", config=None, save_model_checkpoint=False, script_file=None, vae_epochs=8, vae_n_fake_per_task=32, si_lambda=0.5):
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"], "batch_size": cfg["batch_size"],
        "lambda_slice": lambda_slice, "lambda_feat": lambda_feat, "vae_epochs": vae_epochs, "vae_n_fake_per_task": vae_n_fake_per_task, "si_lambda": si_lambda}
                ...
                for x, y in task_loaders[task][0]:
                    ...
                    if task > 0 and vaes:
                        ...
                        if x_fake.size(0) > 0:
                            ...
                            ce_fake = criterion(logits_fake[:, :valid_out_dim], y_fake)
                            loss = loss + 0.5 * ce_fake
...
if __name__ == "__main__":
    run_experiment(run_name="cifar10_vae_si", script_file=os.path.abspath(__file__))
