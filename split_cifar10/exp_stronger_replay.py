# Split CIFAR-10 加强伪回放（更多 VAE 伪样本，对应 MNIST Exp34）
import os
import sys
import copy
import logging
...
from split_cifar10.base_experiment import DEFAULT_CONFIG, DEVICE, DATA_DIR, set_seed, setup_logging, CosineCNN, freeze_model, reg_loss_slice, log_group_differences
from split_cifar10.vae_cifar import CVAE, train_cvae, sample_from_vaes
from split_cifar10.vae_cifar import CIFAR_IN_DIM


def run_experiment(run_name="cifar10_stronger_replay", config=None, save_model_checkpoint=False, script_file=None,
    vae_epochs=10, vae_n_fake_per_task=64, fake_cap=256):
    """更多伪样本、更长 VAE 训练。"""
    ...
    exp_config = {"device": str(DEVICE), "lr": cfg["lr"], "epochs_per_task": cfg["epochs_per_task"], "batch_size": cfg["batch_size"],
        "lambda_slice": lambda_slice, "lambda_feat": lambda_feat, "vae_epochs": vae_epochs, "vae_n_fake_per_task": vae_n_fake_per_task, "fake_cap": fake_cap}
    ...
    with ExperimentManager(run_name, exp_config, script_file=script_file, experiment_group="split_cifar10") as mgr:
        ...
        for task in range(5):
            ...
            vae = CVAE(in_dim=CIFAR_IN_DIM, num_classes=2, latent_dim=64, hidden=512).to(DEVICE)
...
if __name__ == "__main__":
    run_experiment(run_name="cifar10_stronger_replay", script_file=os.path.abspath(__file__),
        vae_epochs=10, vae_n_fake_per_task=64, fake_cap=256)
